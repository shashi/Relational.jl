...
function show(io::IO, x::Select)
    write(io, "SELECT ",
        join(UTF8String[sprint(io->show(io, x)) for x in x.schema.attrs], ", "))
    show(io, x.from)
end

show(io::IO, x::DataStreamRelation) = schema(x).name
...
function show(io::IO, x::QualifiedAttr)
    write(io, string(x.schema.name), ".", string(name(x.attr)))
end
